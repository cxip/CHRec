...
public class MSAPIController {
	private static MSAPIController instance;
	private static final String TAG="ClubhouseAPI";
	private static final boolean DEBUG=BuildConfig.DEBUG;

	private Uri API_URL=Uri.parse("https://www.clubhouseapi.com/api");
//	private static final Uri API_URL=Uri.parse("http://192.168.0.51:8080/");
	...
	private MSAPIController(){
		apiThread=new WorkerThread("ApiThread");
		apiThread.start();
	}

	public static MSAPIController getInstance(){
		if(instance==null){
			instance=new MSAPIController();
		}
		return instance;
	}

	public <T> void execRequest(ClubhouseAPIRequest<T> req){
		API_URL=Uri.parse(req.url);
			...
			try{
				...
				if(req.requestBody!=null){
					if(req.url==null) {
						...
						InstagramAuth.Body body=(InstagramAuth.Body)req.requestBody;
						String formdata="client_id="+body.client_id+"&client_secret="+body.client_secret+"&grant_type=authorization_code&redirect_uri="+body.redirect_uri+"&code="+body.code;
						reqBody = RequestBody.create(MultipartBody.FORM, formdata);
					}
				...
				if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.N) {
					...
					LocaleListCompat locales = getLocales(App.applicationContext.getResources().getConfiguration());
					bldr.header("CH-Languages", locales.toLanguageTags())
							.header("CH-Locale", locales.get(0).toLanguageTag().replace('-', '_'))
							.header("Accept", "application/json")
							.header("CH-AppBuild", API_BUILD_ID)
...
//						T robj=gson.fromJson(body.charStream(), req.responseClass);
						...
						if(req.url==null) {
							...
							req.onError(new ClubhouseErrorResponse(resp.code(), br.errorMessage));
						}else{
							req.onError(new ClubhouseErrorResponse(resp.code(), respStr));
						}
